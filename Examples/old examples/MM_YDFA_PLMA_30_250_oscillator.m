...
sim.deltaZ = 500e-6; % m; 
sim.MPA.M = 4; % MPA parallelization
...
%% Initial pulse
input_field.dt = dt;
input_field.fields = zeros([size(t,1) 8]);

tmp = exp(-2.77*(t/2).^2);
tmp = tmp/sqrt( dt*sum(abs(tmp).^2)*1e-3 )*sqrt(4);
...
input_field.fields(:,7) = tmp;
input_field.fields(:,8) = tmp;
...
%%
[Mfield, dx] = get_modes_fields('D:\GMMNLSE\Fibers\PLMA-YDF-30-250-VIII\', '1030');
filter = exp(-2.77*((lambda-1030)/6).^2);
NPEsat = 22000;
NPEmod = 0.98;
uRT{1} = input_field.fields;
cmap = hsv(size(input_field.fields,2));
saveDir = 'D:\GMMNLSE\SimData\Oscillator_PLMA-YDF-30-250-VIII\sat250nJFilter06nmPH12\';
mkdir(saveDir);
fiber.gain_coeff = 2*log(1000)/fiber.L0*500;
fiber.gain_fwhm = 40e-9;
fiber.saturation_intensity  = 250e-9/(pi*(25e-6/2)^2);
...
%%
    ...
    for RT=1:150
        ...
        %Pinhole
        input_field.fields = get_modes_pinhole(Mfield, input_field.fields, dx, 12e-6, [0e-6 0e-6]);
    ...
    %             ':', 'Color', cmap(ii,:), 'LineWidth', 2);
        ...
        uout = uRT{RT+1};
        save(fullfile(saveDir, fname), 'uout', 'input_field', 'output_field');
        pause(0.2)
        if E(RT)<1e-2
            break
        end
