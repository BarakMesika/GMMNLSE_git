...
fiber.MM_folder = '../Fibers/FG025LJA/';
...
tmp = tmp/sqrt( dt*sum(abs(tmp).^2)*1e-3 )*sqrt(3000);
hold on
plot(t,abs(tmp).^2)

df0 = 20;
seed = fftshift( fft( exp(-2.77*((f-sim.f0-df0)/2).^8) ) );
seed = fft( ifft(seed).*exp(1i*GDD/55*(f-sim.f0-df0).^2) );
seed = seed/sqrt( dt*sum(abs(seed).^2)*1e-3 )*sqrt(1);
plot(fftshift(lambda), abs(fftshift(ifft(seed))).^2)

noise = randn(size(tmp))+1i*randn(size(tmp));
noise = noise/sqrt( dt*sum(abs(noise).^2)*1e-3 )*sqrt(1e-5);


input_field.fields(:,1) = 1*seed+noise;
input_field.fields(:,2) = 1*seed+noise;
input_field.fields(:,3) = 1*seed+noise;
input_field.fields(:,4) = 1*seed+noise;
input_field.fields(:,5) = 1*seed+noise;
input_field.fields(:,6) = 1*tmp+noise;
input_field.fields(:,7) = 1*seed+noise;
input_field.fields(:,8) = 1*seed+noise;
