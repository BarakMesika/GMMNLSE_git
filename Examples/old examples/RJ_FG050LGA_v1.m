...
fiber.MM_folder = '../Fibers/FG050LGA/';
fiber.betas_filename = 'betas.mat';
fiber.S_tensors_filename = 'S_tensors_15modes.mat';
...
gain_rate_eqn.midx = 1:15; % the mode index
gain_rate_eqn.mode_volume = 15; % the total number of available spatial modes in the fiber
...
tmp = sqrt( exp(-2.77*(t/0.8).^2) );
GDD = 0;
tmp = fft( ifft(tmp).*exp(1i*GDD*(f-sim.f0).^2) );
tmp = tmp/sqrt( dt*sum(abs(tmp).^2)*1e-3 )*sqrt(1000);
plot(t, abs(tmp).^2)
...
for ii = 1:100
    ...
    z = output_field.z;
    t = t;
    fname = ['H:\GMMNLSE\Examples\RJ_FG050LGA_Data\data' num2str(ii) '.mat'];
