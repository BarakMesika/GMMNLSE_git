...
gain_rate_eqn.copump_power = 30; % W
...
tmp = exp(-2.77*(t/0.025).^2);
tmp = tmp/sqrt( dt*sum(abs(tmp).^2)*1e-3 )*sqrt(1);
noise = randn(size(tmp))+1i*randn(size(tmp));
noise = noise/sqrt( dt*sum(abs(noise).^2)*1e-3 )*sqrt(1e4);

ModeE = rand(gain_rate_eqn.mode_volume,1)*250;
GDD = 2;
...
for ic=10:12
    cw = 10-(9-1);
    GDD = 80;
    filter1 = exp(-2.77*((lambda-(1050-cw))/1.0).^2).*exp(1i*GDD*(f-sim.f0).^2);
    filter2 = exp(-2.77*((lambda-(1050+cw))/1.0).^2).*exp(1i*GDD*(f-sim.f0).^2);
%     plot(fftshift(lambda), fftshift(filter1), fftshift(lambda), fftshift(filter2))
    uamp = input_field.fields;
    %%
    dirName  = ['MO_XLMA_YTF_100_400_480_data\Filter_' num2str(ic) '\'];
    mkdir(dirName);
    data.t = t;
    data.lambda = lambda;
    ...
    for RT = 1:60
        ...
        subplot(2,1,2)
        plot(fftshift(lambda), abs(fftshift(ifft(uamp,[],1),1)).^2, 'LineWidth', 2);xlim([1000 1100]);
