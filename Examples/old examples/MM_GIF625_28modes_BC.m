...
fiber.MM_folder = '../Fibers/GIF625_28modes_CW1030nm/';
fiber.betas_filename = 'betas.mat';
fiber.S_tensors_filename = 'S_tensors_28modes.mat';
...
gain_rate_eqn.midx = 1:28; % the mode index
gain_rate_eqn.mode_volume = 28; % the total number of available spatial modes in the fiber
gain_rate_eqn.downsampling_factor = 4; % downsample the eigenmode profiles to run faster
...
fiber.L0 = 1.00; % m; the length of the gain fiber
save_num = 50;
...
noise = noise/sqrt( dt*sum(abs(noise).^2)*1e-3 )*sqrt(0.01);
...
E_LP01 = 1/15;
M_num = 14;
E_modes = ones(M_num,1);
E_modes = [E_modes; zeros(gain_rate_eqn.mode_volume-M_num,1)];
...
%%
    ...
    dirName  = 'GIF625_10modes_CW1030nm_SimData\';
    mkdir(dirName);
    data.t = t;
    data.lambda = lambda;
