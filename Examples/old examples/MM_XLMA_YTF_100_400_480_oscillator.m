...
fiber.S_tensors_filename = 'S_tensors_25modes.mat';
...
gain_rate_eqn.core_diameter = 94; % um
...
gain_rate_eqn.midx = 1:25; % the mode index
gain_rate_eqn.mode_volume = 25; % the total number of available spatial modes in the fiber
...
%% Initial pulse
input_field.dt = dt;
input_field.fields = zeros([size(t,1) 25]);
...
%%
[Mfield, dx] = get_modes_fields('G:\GMMNLSE\Fibers\XLMA-YTF-100-400-480\', '1030');
filter = exp(-2.77*((lambda-1030)/15).^2);
NPEsat = 15000;
NPEmod = 0.98;
uRT{1} = input_field.fields;
cmap = lines(size(input_field.fields,2));
saveDir = 'G:\GMMNLSE\SimData\Oscillator_XLMA-YTF-100-400-480\PH020um_sat750\';
mkdir(saveDir);
fiber.gain_coeff = 2*log(1000)/fiber.L0*500;
fiber.gain_fwhm = 40e-9;
fiber.saturation_intensity  = 750e-9/(pi*(25e-6/2)^2);

SimParam = MMSimParam();
SimParam.Modes = Mfield;
SimParam.dx = dx;
SimParam.time = t;
SimParam.dt = dt;
SimParam.f = f;
SimParam.wl = lambda;
SimParam.wl0 = sim.lambda0;
SimParam.c = c;
SimParam.N = N;
%%
    uRT{1} = input_field.fields;
    for RT=1:100
        ...
        %Pinhole
        input_field.fields = get_modes_pinhole(Mfield, input_field.fields, dx, 20e-6, [0e-6 0e-6]);
    ...
    %             ':', 'Color', cmap(ii,:), 'LineWidth', 2);
        ...
        saveas(gcf, fullfile(saveDir, fname), 'tiffn');
        saveas(gcf, fullfile(saveDir, fname), 'fig');
        uout = uRT{RT+1};
        save(fullfile(saveDir, fname), 'uout', 'input_field');
        pause(0.2)
        if E(RT)<1e-2
            break
        end
