...
gain_rate_eqn.absorption_to_get_N_total = 8.75; % dB/m
gain_rate_eqn.pump_wavelength = 976; % nm
gain_rate_eqn.copump_power = 9.25; % W
...
% tmp = exp(-2.77*((t-0)/0.15).^2);
% tmp = tmp/sqrt( dt*sum(abs(tmp).^2)*1e-3 )*sqrt(1);
noise = randn(size(t))+1i*randn(size(t));
noise = noise/sqrt( dt*sum(abs(noise).^2)*1e-3 )*sqrt(1e-3);

load('MO_seed.mat');
input_field.fields = uamps+noise;

% ModeE = ones(gain_rate_eqn.mode_volume,1)*1;
% GDD = 0;
...
%%
    ...
    cw = 5;
    GDD = 0;
    filter1 = exp(-2.77*((lambda-(1030-cw))/3.0).^2).*exp(1i*GDD*(f-sim.f0).^2);
    filter2 = exp(-2.77*((lambda-(1030+cw))/3.0).^2).*exp(1i*GDD*(f-sim.f0).^2);
%     plot(fftshift(lambda), fftshift(filter1), fftshift(lambda), fftshift(filter2))
    uamp = input_field.fields;
    %%
    dirName  = ['GRIN30Yb\Filter4_' num2str(cw) '_P09p25\'];
    mkdir(dirName);
    data.t = t;
    data.lambda = lambda;
    ...
    for RT = 1:60
        ...
        fName = ['data1_RT' num2str(RT)];
        save([dirName fName], 'uamps');
        Etot(1,RT) = sum( sum(abs(uamp).^2,1) )*dt/1e3*1e-3;
        fprintf('E:%d\n', Etot(1,RT));

        subplot(2,1,1)
        plot(t, abs(uamp).^2, 'LineWidth', 2);xlim([min(t) max(t)]);
        subplot(2,1,2)
        plot(fftshift(lambda), abs(fftshift(ifft(uamp,[],1),1)).^2, 'LineWidth', 2);xlim([970 1100]);

        input_field.fields = fft( bsxfun(@times,filter2, ifft( uamp,[],1) ),[], 1 );
        output_field = GMMNLSE_propagate(fiber,input_field,sim,gain_param);
        uamp = output_field.fields(:,:,end);
        uamps = single(output_field.fields);
        fName = ['data2_RT' num2str(RT)];
        save([dirName fName], 'uamps');
        Etot(2,RT) = sum( sum(abs(uamp).^2,1) )*dt/1e3*1e-3;
        fprintf('E:%d\n', Etot(2,RT));
        fprintf('RT:%d\n', RT);
