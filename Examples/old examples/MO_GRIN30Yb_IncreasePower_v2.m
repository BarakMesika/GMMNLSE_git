...
fiber.MM_folder = '../Fibers/GRIN30Yb/';
fiber.betas_filename = 'betas.mat';
fiber.S_tensors_filename = 'S_tensors_12modes.mat';
...
gain_rate_eqn.core_diameter = 43*2; % um
...
gain_rate_eqn.absorption_to_get_N_total = 90.0; % dB/m
gain_rate_eqn.pump_wavelength = 976; % nm
gain_rate_eqn.copump_power = 16.5; % W
gain_rate_eqn.counterpump_power = 0; % W
gain_rate_eqn.midx = 1:12; % the mode index
gain_rate_eqn.mode_volume = 12; % the total number of available spatial modes in the fiber
...
fiber.L0 = 2.5; % m; the length of the gain fiber
save_num = 100;
...
sim.gain_model = 4;
% sim.gain_model = 2;
% fiber.gain_coeff = 8*log(1000)/fiber.L0/100000000000000;
...
% 
% load('MO_seed.mat');
% input_field.fields = uamps(:,:,end)+noise;

ModeE = ones(gain_rate_eqn.mode_volume,1)*5;
ModeE(2:end) = ModeE(2:end)/4;
GDD = 0;
for ii=1:gain_rate_eqn.mode_volume
%     input_field.fields(:,ii) = fft( ifft(tmp).*exp(1i*GDD*(f-sim.f0).^2) ).*sqrt(ModeE(ii))+noise;
    input_field.fields(:,ii) = tmp*sqrt(ModeE(ii))+noise;
end
...
%%
    ...
    cw = 7;
    GDD = 0;
    filter1 = exp(-2.77*((lambda-(1055-cw))/4.0).^2).*exp(1i*GDD*(f-sim.f0).^2);
    filter2 = exp(-2.77*((lambda-(1055+cw))/4.0).^2).*exp(1i*GDD*(f-sim.f0).^2);
%     plot(fftshift(lambda), fftshift(filter1), fftshift(lambda), fftshift(filter2))
    uamp = input_field.fields;
    %%
    dirName  = ['GRIN30Yb\Pump_Up_2\'];
    mkdir(dirName);
    data.t = t;
    data.lambda = lambda;
    ...
    %%
    PowerIterMax = 100;
    for PowerIter = 1:PowerIterMax
        RT = 1;  
        flag = inf;
        flagR = 0;
        while RT < 50
            input_field.fields = fft( bsxfun(@times,filter1, ifft( uamp,[],1) ),[], 1 );
            output_field = GMMNLSE_propagate(fiber,input_field,sim,gain_param);
            uamp = output_field.fields(:,:,end);
            uamps = single(output_field.fields);
            fName = ['data1_RT' num2str(RT) '_Pump_' num2str(gain_rate_eqn.copump_power*100)];
            save([dirName fName], 'uamps');
            Etot(1,RT) = sum( sum(abs(uamp).^2,1) )*dt/1e3;
            fprintf('E:%d 1st Arm\n', Etot(1,RT));

            subplot(2,1,1)
            plot(t, abs(uamp).^2, 'LineWidth', 2);xlim([min(t) max(t)]);
            subplot(2,1,2)
            plot(fftshift(lambda), abs(fftshift(ifft(uamp,[],1),1)).^2, 'LineWidth', 2);xlim([970 1200]);

            input_field.fields = fft( bsxfun(@times,filter2, ifft( uamp,[],1) ),[], 1 );
            output_field = GMMNLSE_propagate(fiber,input_field,sim,gain_param);
            uamp = output_field.fields(:,:,end);
            uamps = single(output_field.fields);
            fName = ['data2_RT' num2str(RT) '_Pump_' num2str(gain_rate_eqn.copump_power*100)];
            save([dirName fName], 'uamps');
            Etot(2,RT) = sum( sum(abs(uamp).^2,1) )*dt/1e3;
            fprintf('E:%d  2nd Arm\n', Etot(2,RT));
            ...
            fprintf('RT:%d done flag:%0.4f Pump:%d\n', RT, flag, gain_rate_eqn.copump_power);
            if RT>1; flag = abs( Etot(2,RT-1)-Etot(2,RT) )/abs(Etot(2,RT));end
            RT = RT + 1;
            if flag<5e-3
                flagR = flagR+1;
            else
        ...
        gain_rate_eqn.copump_power = gain_rate_eqn.copump_power+0.2; % W
        [gain_rate_eqn,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN] = gain_info( sim,gain_rate_eqn,lambda );
        gain_param = {gain_rate_eqn,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN}; % send this into GMMNLSE_propagate function
