...
% YB1200-10-125DC, which enables modes other than the fundamental modes to
% leak into the cladding (bad confinement for higher-order modes).
%
% Because of the bad confinement, the gain that higher-order modes acquire
% is smaller than that the fundamental mode has and therefore fundamental
% mode grows faster than the other higher-order modes.
%
...
gain_rate_eqn.saved_mat_filename = 'MM_YDFA_weak_waveguide_rate_eqn'; % the files used to temporarily store the data during iterations if necessary
...
gain_Aeff = 8.70538e-11; % m^2; the Aeff of the first mode
fiber.t_rep = gain_rate_eqn.t_rep; % for calculating saturation intensity
save_num = 100;
...
fiber_SMgain = fiber;
fiber_SMgain.db_gain = 36.6; % the small-signal dB gain
fiber_SMgain.saturation_intensity = 35; % J/m^2
...
fiber_newgain = fiber;
fiber_newgain.db_gain = 36.6; % the small-signal dB gain
fiber_newgain.saturation_intensity = 35; % J/m^2
...
%% Save results
save('MM_YDFA_weak_waveguide.mat');