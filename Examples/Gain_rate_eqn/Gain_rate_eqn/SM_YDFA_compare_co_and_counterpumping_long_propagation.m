...
gain_rate_eqn.saved_mat_filename = 'SM_YDFA_compare_co_and_counterpumping_long_propagation'; % the files used to temporarily store the data during iterations if necessary
...
sim.adaptive_deltaZ.model = 0; % don't use adaptive-step method
                               % Because adaptive-step method supports only copumping, not counterpumping.
                               % To be consistent, I turn it off for both cases.
sim.deltaZ = 250e-6; % m
sim.gpu_yes = false;

sim.gain_model = 4; % use rate-equation-gain model
...
input_field.ASE_forward = zeros(N,1);
input_field.ASE_backward = zeros(N,1);
...
% case 1: copumping
[gain_rate_eqn_copumping,cross_sections_pump,cross_sections,overlap_factor,N_total] = gain_info( sim,gain_rate_eqn_copumping,lambda );
gain_param_copumping = {gain_rate_eqn_copumping,cross_sections_pump,cross_sections,overlap_factor,N_total}; % send this into GMMNLSE_propagate function
% case 2: counterpumping
[gain_rate_eqn_counterpumping,cross_sections_pump,cross_sections,overlap_factor,N_total] = gain_info( sim,gain_rate_eqn_counterpumping,lambda );
gain_param_counterpumping = {gain_rate_eqn_counterpumping,cross_sections_pump,cross_sections,overlap_factor,N_total}; % send this into GMMNLSE_propagate function

gain_param = {gain_param_copumping,gain_param_counterpumping};
...
for i = 1:2
    t_start = tic;
    output_field{i} = GMMNLSE_propagate(fiber,input_field,sim,gain_param{i});
    t_end(i) = toc(t_start);
    t_spent = datevec(t_end(i)/3600/24);
    fprintf('Running time for %s: %2u:%2u:%3.1f\n',model_name{i},t_spent(4),t_spent(5),t_spent(6));
end

%% Plot results
% nonlinear phase
z = 0:sim.save_period:fiber.L0;
nonlinear_phase_copumping = accumulated_nonlinear_phase(fiber.L0,1/fiber.SR,sim.f0,output_field{1}.fields,z,output_field{1}.dt);
nonlinear_phase_counterpumping = accumulated_nonlinear_phase(fiber.L0,1/fiber.SR,sim.f0,output_field{2}.fields,z,output_field{2}.dt);
...
%% Save results
save('SM_YDFA_compare_co_and_counterpumping_long_propagation.mat');