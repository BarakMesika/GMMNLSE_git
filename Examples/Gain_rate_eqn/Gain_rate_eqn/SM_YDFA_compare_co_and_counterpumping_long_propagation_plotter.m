...
filename = 'SM_YDFA_compare_co_and_counterpumping_long_propagation.mat';
...
factor_correct_unit = (N*dt)^2/1e3;
...
figure;
subplot(2,1,1);
h = plot(t,abs(output_field{1}.fields(:,:,end)).^2/1e3);
xlabel('Time (ps)');
ylabel('Intensity (kW)');
...
% Spectrum
subplot(2,1,2);
h = plot(lambda,abs(fftshift(ifft(output_field{1}.fields(:,:,end)),1)).^2.*factor*factor_correct_unit);
...
figure;
subplot(2,1,1);
h = plot(t,abs(output_field{2}.fields(:,:,end)).^2/1e3);
xlabel('Time (ps)');
ylabel('Intensity (kW)');
...
% Spectrum
subplot(2,1,2);
h = plot(lambda,abs(fftshift(ifft(output_field{2}.fields(:,:,end)),1)).^2.*factor*factor_correct_unit);
xlabel('Wavelength (nm)');
ylabel('Intensity (nJ/nm)');
