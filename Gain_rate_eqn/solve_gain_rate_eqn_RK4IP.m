function varargout = solve_gain_rate_eqn_RK4IP(direction,...
                                               sim,gain_rate_eqn,...
...
%SOLVE_GAIN_RATE_EQN_RK4IP Solves the power of pump, ASE, and signal at
...
if ~first_backward_before_iterations
    ...
    if sim.gpu_yes
        if sim.single_yes
            AmAn = zeros(1,1,num_spatial_modes,num_spatial_modes,N,M,'single','gpuArray');
        else
            AmAn = zeros(1,1,num_spatial_modes,num_spatial_modes,N,M,'gpuArray');
        end
    else
        if sim.single_yes
            AmAn = zeros(1,1,num_spatial_modes,num_spatial_modes,N,M,'single');
        else
...
N2 = solve_N2( sim,gain_rate_eqn,N_total,E_photon,diag_idx,overlap_factor,cross_sections_pump,cross_sections,Power_pump_forward,Power_pump_backward,Pdf_ASE_forward,Pdf_ASE_backward,AmAn,first_backward_before_iterations ); % unit: 1/um^3
...
switch direction
    case 'forward'
        ...
        [~,G]                  = solve_Power( 'signal',sim.scalar,deltaZ*1e6,dx,A_core,overlap_factor.signal,cross_sections,     N2,N_total,                 [],       [],     [], FmFnN,                        []); % no spontaneous term for signal
    case 'backward'
        % at z-deltaZ
        ...
        if ~gain_rate_eqn.ignore_ASE
            Power_ASE_backward = solve_Power( 'ASE',   sim.scalar,deltaZ*1e6,dx,A_core,overlap_factor.signal,cross_sections,     N2,N_total,Power_ASE_backward,  E_photon,     [], FmFnN, gain_rate_eqn.mode_volume);
        end
end

% Change the size back to (N,num_modes)
if isequal(direction,'forward')
    if sim.adaptive_deltaZ.model % use adaptive method
...
switch direction
    case 'forward'
        if gain_rate_eqn.export_N2
            ...
            varargout = {Power_pump_forward,Power_ASE_forward,G};
        end
    case 'backward'
...
%%
function [Pnext,signal_out] = solve_Power( field_type,isscalar,deltaZ,dx,A_core,overlap_factor,cross_sections,N2,N_total,P0,E_photon,GammaN,FmFnN,mode_volume)
...
if isempty(dx) % fundamental mode
    ...
    switch field_type
        case 'signal' % ignore spontaneous term
            ...
            % Calculate gA after passing through the gain.
            signal_out = permute( 1e6/2*(cross_section_all.*FmFnN2 - cross_sections.absorption.*FmFnN), [1,2,4,3,5,6,7]); % Am_after_gain - Am
                                                                                                                           % 1e6 is due to deltaZ having the unit of "m" while the term here is meant for "um" deltaZ.
        case 'ASE'
            num_modes = size(P0,3);
