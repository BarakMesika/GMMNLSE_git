function varargout = GMMNLSE_MPA_step_MMrategain_linear_oscillator(A0, A0_backward, Power_pump_forward, Power_pump_backward, Power_ASE_forward, Power_ASE_backward, dt, sim, prefactor, SRa_info, SRb_info, SK_info, omegas, D, haw, hbw, gain_rate_eqn, cross_sections_pump, cross_sections, overlap_factor, N_total, FmFnN, GammaN)
%GMMNLSE_MPA_STEP_MMRATEGAIN Take one step according to the GMMNLSE, with a
%gain model solved from rate equations. It's only used for multimode
%cases and the gain term is treated as a nonlinear term like the normal new
%gain we use, instead of a dispersion term in MPA. Because of this, the
%step(sim.deltaZ*M) needs to be small enough for MPA to converge.
%
...
for n_it = 1:sim.MPA.n_tot_max
    ...
    % If not using the GPU, we will precompute Ra_mn, Rb_mn before the num_modes^4 sum
    ...
    if gain_rate_eqn.export_N2
        [Power_pump_forward,Power_pump_backward,Power_ASE_forward,gain_term,N2] = solve_gain_rate_eqn_linear_oscillator(sim,gain_rate_eqn,A_w,A0_backward,Power_pump_forward,Power_pump_backward,Power_ASE_forward,Power_ASE_backward,omegas,dt,sim.deltaZ,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN);
    else
        [Power_pump_forward,Power_pump_backward,Power_ASE_forward,gain_term]    = solve_gain_rate_eqn_linear_oscillator(sim,gain_rate_eqn,A_w,A0_backward,Power_pump_forward,Power_pump_backward,Power_ASE_forward,Power_ASE_backward,omegas,dt,sim.deltaZ,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN);
    end
    ...
    nonlinear = permute(ifft(nonlinear),[1 3 2]); % (N,num_modes,M+1)
    ...
    nonzero_idx = find(max(abs(last_psi))~=0);
    if gain_rate_eqn.allow_coupling_to_zero_fields
        if sim.single_yes
            error('If allowing coupling to all-zero fields, "double-precisoin" is necessary, or it''ll be hard to converge for MPA.');
        end
        idx_tol = 1:num_modes; % keep all the fields, including all-zero fields
    else
        idx_tol = nonzero_idx; % keep only the nonzero fields
        
        zero_idx = max(abs(last_psi))==0;
        last_psi = last_psi(:,idx_tol);
        psi(:,zero_idx,:) = 0; % clear effects from the gain model by making them all zeros
    end
    current_psi = psi(:,idx_tol,sim.MPA.M+1);
    ...
    if n_it == sim.MPA.n_tot_max
        if gain_rate_eqn.allow_coupling_to_zero_fields
            error('Error in GMMNLSE_MPA_step: The step did not converge after %d iterations, aborting.\nPlease increase "sim.n_tot_max" because the coupling to all-zero fields is allowed and this will make MPA difficult to converge.\nOr it''ll possibly be better to turn off the coupling to all-zero fields.', sim.n_tot_max);
        else
...
A1_right_end = D.pos(:,:,sim.MPA.M+1).*psi(:,:,sim.MPA.M+1);

if gain_rate_eqn.export_N2
    varargout = {num_it_tot, A1_right_end, Power_pump_forward(:,:,:,:,:,end), Power_pump_backward(:,:,:,:,:,end), Power_ASE_forward(:,:,:,:,:,end), N2(:,:,:,:,:,:,end)};
else
    varargout = {num_it_tot, A1_right_end, Power_pump_forward(:,:,:,:,:,end), Power_pump_backward(:,:,:,:,:,end), Power_ASE_forward(:,:,:,:,:,end)};
end
