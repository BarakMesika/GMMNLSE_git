function varargout = GMMNLSE_ss_step_rategain_linear_oscillator(A0, A0_backward, Power_pump_forward, Power_pump_backward, Power_ASE_forward, Power_ASE_backward, dt, sim, prefactor, SRa_info, SRb_info, SK_info, omegas, D, haw, hbw, gain_rate_eqn, cross_sections_pump, cross_sections, overlap_factor, N_total, FmFnN, GammaN)
%GMMNLSE_SS_STEP_SMGAIN Take one step according to the GMMNLSE, with a
%gain model solved from rate equations. It's only used for fundamental-mode
%cases and the gain term is treated as a dispersion term like the normal SM
%gain we use, instead of a nonlinear term.
%
...
if gain_rate_eqn.export_N2
    [Power_pump_forward,Power_pump_backward,Power_ASE_forward,G,N2] = solve_gain_rate_eqn_linear_oscillator(sim,gain_rate_eqn,A0,A0_backward,Power_pump_forward,Power_pump_backward,Power_ASE_forward,Power_ASE_backward,omegas,dt,sim.deltaZ,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN);
else
    [Power_pump_forward,Power_pump_backward,Power_ASE_forward,G]    = solve_gain_rate_eqn_linear_oscillator(sim,gain_rate_eqn,A0,A0_backward,Power_pump_forward,Power_pump_backward,Power_ASE_forward,Power_ASE_backward,omegas,dt,sim.deltaZ,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN);
end
...
k1 = sim.deltaZ*get_dA_dz(A_t     , dt, sim, SRa_info, SRb_info, SK_info, haw, hbw, prefactor, N, num_modes, Kerr, Ra, Rb, anisotropic_Raman_included);
k2 = sim.deltaZ*get_dA_dz(A_t+k1/2, dt, sim, SRa_info, SRb_info, SK_info, haw, hbw, prefactor, N, num_modes, Kerr, Ra, Rb, anisotropic_Raman_included);
k3 = sim.deltaZ*get_dA_dz(A_t+k2/2, dt, sim, SRa_info, SRb_info, SK_info, haw, hbw, prefactor, N, num_modes, Kerr, Ra, Rb, anisotropic_Raman_included);
k4 = sim.deltaZ*get_dA_dz(A_t+k3  , dt, sim, SRa_info, SRb_info, SK_info, haw, hbw, prefactor, N, num_modes, Kerr, Ra, Rb, anisotropic_Raman_included);

A_w_afternonlin = ifft(A_t + (k1+2*(k2+k3)+k4)/6);

% 3) Propagate through the second dispersion + gain section
A1 = DG.*A_w_afternonlin;

if gain_rate_eqn.export_N2
    varargout = {A1, Power_pump_forward,Power_pump_backward,Power_ASE_forward, N2};
else
    varargout = {A1, Power_pump_forward,Power_pump_backward,Power_ASE_forward};
end

end

function dA_dz = get_dA_dz(A_t, dt, sim, SRa_info, SRb_info, SK_info, haw, hbw, prefactor, N, num_modes, Kerr, Ra, Rb, anisotropic_Raman_included)
...
if sim.gpu_yes
    ...
    %            T_mn(:, midx3, midx4) = A_t(:, midx3).*conj(A_t(:, midx4));
    %        end
    %    
    ...
    %        Ra(:, midx1, midx2) = Ra(:, midx1, midx2) + SRa_info.SRa(nz_idx)*T_mn(:, midx3, midx4);
    %    end
    % -----------------------------------------------------------------
...
dA_dz = fft(prefactor.*ifft(nonlinear));
