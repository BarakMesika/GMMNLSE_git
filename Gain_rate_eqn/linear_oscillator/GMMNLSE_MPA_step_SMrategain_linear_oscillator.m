function varargout = GMMNLSE_MPA_step_SMrategain_linear_oscillator(A0, A0_backward, Power_pump_forward, Power_pump_backward, Power_ASE_forward, Power_ASE_backward, dt, sim, prefactor, SRa_info, SRb_info, SK_info, omegas, D_op, haw, hbw, gain_rate_eqn, cross_sections_pump, cross_sections, overlap_factor, N_total, FmFnN, GammaN)
%GMMNLSE_MPA_STEP_RATEGAIN Take one step according to the GMMNLSE, with a
...
%gain we use, instead of a nonlinear term in MPA.
%
...
for n_it = 1:sim.MPA.n_tot_max
    ...
    cumulative_gz = 0; % Keep track of the total gain, determined by the energy at each small step
    for ii = 2:sim.MPA.M+1
        % Update A_w, Power_pump_forward, and Power_ASE_forward for each ii
        A_w_ii = permute(A_w(:,ii-1,:),[1 3 2]); % (N,num_modes)
        if ii == 2
            ...
            last_Power_pump_forward = Power_pump_forward_ii;
            last_Power_ASE_forward = Power_ASE_forward_ii;
            last_Power_pump_backward = Power_pump_backward_ii;
        end
        
        % Gain and Dispersion
        if gain_rate_eqn.export_N2
            [Power_pump_forward_ii,Power_pump_backward_ii,Power_ASE_forward_ii,G,N2] = solve_gain_rate_eqn_linear_oscillator(sim,gain_rate_eqn,A_w_ii,A0_backward,last_Power_pump_forward,last_Power_pump_backward,last_Power_ASE_forward,Power_ASE_backward,omegas,dt,sim.deltaZ,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN);
        else
            [Power_pump_forward_ii,Power_pump_backward_ii,Power_ASE_forward_ii,G]    = solve_gain_rate_eqn_linear_oscillator(sim,gain_rate_eqn,A_w_ii,A0_backward,last_Power_pump_forward,last_Power_pump_backward,last_Power_ASE_forward,Power_ASE_backward,omegas,dt,sim.deltaZ,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN);
        end
        gz = log(G); % E = G*E0 = exp(g*deltaZ)*E0. The G here has already been multiplied by "deltaZ".
        cumulative_gz = cumulative_gz + gz;
        D.pos(:,:,ii) = exp((ii-1)*D_op + cumulative_gz);
    ...
    if sim.gpu_yes
        ...
        if sim.single_yes
            %Ra_mn = complex(zeros(N, sim.MPA.M+1, num_modes, num_modes, 'single'));
            Kerr = complex(zeros(N, sim.MPA.M+1, num_modes, 'single'));
            Ra = complex(zeros(N, sim.MPA.M+1, num_modes, num_modes, 'single'));
        else
            %Ra_mn = complex(zeros(N, sim.MPA.M+1, num_modes, num_modes));
...
if gain_rate_eqn.export_N2
    varargout = {num_it_tot, A1_right_end, Power_pump_forward_ii,Power_pump_backward_ii,Power_ASE_forward_ii, N2};
else
    varargout = {num_it_tot, A1_right_end, Power_pump_forward_ii,Power_pump_backward_ii,Power_ASE_forward_ii};
end
