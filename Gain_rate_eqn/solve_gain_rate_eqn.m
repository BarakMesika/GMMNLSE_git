...
if sim.gpu_yes && isequal(sim.step_method,'MPA')
...
if isequal(field_type,'signal')
    ...
    if isempty(dx) % fundamental mode
        ...
        % For multimode cases, MPA is used. The gain is treated as an
        % nonlinear term, so Pnext considers multiple z-planes.
        % Therefore, parallization M=size(fz) can't be one in this code.
        %
        % Pnext = trapz(z=(1:M)*dz,fz) with Pi=P0(1) and Pf=P0(end)+fz(end)
        fz(:,:,:,:,:,1) = fz(:,:,:,:,:,1)/2;
        if cumprod(size(P0)) == 1
...
%MYTRAPZFFTSHIFT5 Summary of this function goes here
%   "trapz" and "fftshift" are slower on GPU than on CPU when the matrix is
%not large, so for slightly multimode case, it can still be faster for
%CPU.
