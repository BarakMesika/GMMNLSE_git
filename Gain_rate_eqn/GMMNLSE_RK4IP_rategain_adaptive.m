function varargout = GMMNLSE_RK4IP_rategain_adaptive(A0, dt, sim, gain_rate_eqn,...
                                                     SK_info, SRa_info, SRb_info,...
                                                     haw, hbw,...
                                                     prefactor, omegas, D_op,...
                                                     cross_sections_pump, cross_sections, overlap_factor, N_total, FmFnN, GammaN,...
                                                     Power_pump_forward,a5_1,...
                                                     dummy_var)
%GMMNLSE_RK4IP_RATEGAIN_ADAPTIVE Take one step according to the GMMNLSE, 
%with a gain model solved from rate equations. The gain term is treated as 
%a dispersion term like the normal SM gain we use, instead of a nonlinear 
%term.
%
...
% 1) Represented under the interaction picture (dispersion + gain)
Dz = D_op*sim.deltaZ/2;
if gain_rate_eqn.export_N2
[Power_pump_forward,~,G,N2] = solve_gain_rate_eqn_RK4IP('forward',sim,gain_rate_eqn,A0,dummy_var,Power_pump_forward,dummy_var,dummy_var,dummy_var,omegas,dt,sim.deltaZ,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN,false );
else
[Power_pump_forward,~,G]    = solve_gain_rate_eqn_RK4IP('forward',sim,gain_rate_eqn,A0,dummy_var,Power_pump_forward,dummy_var,dummy_var,dummy_var,omegas,dt,sim.deltaZ,cross_sections_pump,cross_sections,overlap_factor,N_total,FmFnN,GammaN,false );
end
...
if isempty(a5_1) || ~isequal(gain_rate_eqn.midx,1) % not fundamental mode
    a5_1 = N_op(        A0,                     dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, G, N, num_modes);
end
a1 = expDG.*a5_1;
a2 =       N_op(        A_IP+a1*(sim.deltaZ/2), dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, G, N, num_modes);
a3 =       N_op(        A_IP+a2*(sim.deltaZ/2), dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, G, N, num_modes);
a4 =       N_op(expDG.*(A_IP+a3*(sim.deltaZ)),  dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, G, N, num_modes);

A1 = expDG.*(A_IP + (a1+2*a2+2*a3)*(sim.deltaZ/6)) + a4*(sim.deltaZ/6);

% 3) Local error estimate
a5 =       N_op(        A1,                     dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, G, N, num_modes);
