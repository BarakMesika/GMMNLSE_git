function [num_it_tot, A1_right_end] = GMMNLSE_MPA_nogain(A0, dt, sim, prefactor, SRa_info, SRb_info, SK_info, D, haw, hbw)
%GMMNLSE_MPA_NOGAIN Take one step according to the GMMNLSE, without 
%gain
%
...
% sim.lmc.model - 0,1,2,3 = the model of (random or deterministic) linear mode coupling used
...
% With mode coupling lmc_model 2,3:
...
for n_it = 1:sim.MPA.n_tot_max
    ...
    %    A_w = D.pos*psi
    if sim.lmc.model==2 % use linear mode coupling: lmc_model 2
        if sim.gpu_yes % GPU
    ...
    if sim.lmc.model == 2
        if sim.gpu_yes
            ...
            nonlinear = permute(nonlinear,[2 4 1 3]); % (num_modes,1,N,M+1)
            nonlinear = sim.deltaZ*multbslash(D.pos, nonlinear);
            nonlinear = permute(nonlinear,[3 1 4 2]); % (N, num_modes, M+1)
        end
    else
