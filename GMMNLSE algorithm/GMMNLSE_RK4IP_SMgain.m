function A1 = GMMNLSE_RK4IP_SMgain(A0, dt, sim, prefactor, SRa_info, SRb_info, SK_info, D, haw, hbw, G, saturation_energy)
%GMMNLSE_RK4IP_SMGAIN Take one step according to the GMMNLSE, with a total energy saturating gain model
...
N = size(A0, 1); % number of time/freq pointsf
num_modes = size(A0, 2);
...
expDG = exp(D + G/(1+total_energy/saturation_energy));
A_IP = expDG.*A0;

% 2) Propagate through the nonlinearity
a1 = expDG.*N_op(        A0,        dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, N, num_modes)*sim.deltaZ;
a2 =        N_op(        A_IP+a1/2, dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, N, num_modes)*sim.deltaZ;
a3 =        N_op(        A_IP+a2/2, dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, N, num_modes)*sim.deltaZ;
a4 =        N_op(expDG.*(A_IP+a3),  dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, N, num_modes)*sim.deltaZ;
