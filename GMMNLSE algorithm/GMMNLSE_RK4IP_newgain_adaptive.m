function [A1,a5,...
          opt_deltaZ,success] = GMMNLSE_RK4IP_newgain_adaptive(A0, dt, sim, prefactor, SRa_info, SRb_info, SK_info, D_op, haw, hbw, a5_1, G, saturation_intensity, fr)
%GMMNLSE_RK4IP_NEWGAIN_ADAPTIVE Take one step according to the GMMNLSE, with a spatially saturating gain model
...
if isempty(a5_1)
    a5_1 = N_op(       A0,        dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, polar, saturation_intensity, transfer_matrix, fr, recovery_idx, G, N, num_modes, num_spatial_modes);
end
a1 = expD.*a5_1;
a2 =       N_op(       A_IP+a1*(sim.deltaZ/2), dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, polar, saturation_intensity, transfer_matrix, fr, recovery_idx, G, N, num_modes, num_spatial_modes);
a3 =       N_op(       A_IP+a2*(sim.deltaZ/2), dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, polar, saturation_intensity, transfer_matrix, fr, recovery_idx, G, N, num_modes, num_spatial_modes);
a4 =       N_op(expD.*(A_IP+a3*(sim.deltaZ)),  dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, polar, saturation_intensity, transfer_matrix, fr, recovery_idx, G, N, num_modes, num_spatial_modes);

A1 = expD.*(A_IP + (a1+2*a2+2*a3)*(sim.deltaZ/6)) + a4*(sim.deltaZ/6);

% 3) Local error estimate
a5 =       N_op(       A1,        dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, polar, saturation_intensity, transfer_matrix, fr, recovery_idx, G, N, num_modes, num_spatial_modes);
