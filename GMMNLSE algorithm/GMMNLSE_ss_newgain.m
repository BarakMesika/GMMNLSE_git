function A1 = GMMNLSE_ss_newgain(A0, dt, sim, prefactor, SRa_info, SRb_info, SK_info, D, haw, hbw, G, saturation_intensity, fr)
%GMMNLSE_SS_STEP_NEWGAIN Take one step according to the GMMNLSE, with a spatially saturating gain model
...
% 2) Propagate through the nonlinearity with RK4, and save the transfer matrices calculated
k1 = N_op(A_t,      dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, polar, saturation_intensity, transfer_matrix, fr, recovery_idx, G, N, num_modes, num_spatial_modes)*sim.deltaZ;
k2 = N_op(A_t+k1/2, dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, polar, saturation_intensity, transfer_matrix, fr, recovery_idx, G, N, num_modes, num_spatial_modes)*sim.deltaZ;
k3 = N_op(A_t+k2/2, dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, polar, saturation_intensity, transfer_matrix, fr, recovery_idx, G, N, num_modes, num_spatial_modes)*sim.deltaZ;
k4 = N_op(A_t+k3,   dt, sim, SK_info, SRa_info, SRb_info, Kerr, Ra, Rb, haw, hbw, anisotropic_Raman_included, prefactor, polar, saturation_intensity, transfer_matrix, fr, recovery_idx, G, N, num_modes, num_spatial_modes)*sim.deltaZ;

A_w_afternonlin = ifft(A_t + (k1+2*(k2+k3)+k4)/6);

% 3) Propagate through the second dispersion section
A1 = exp(D).*A_w_afternonlin;
...
function dAdz = N_op(A_t, dt, sim,...
                   ...
                   prefactor, polar,...
                   saturation_intensity, transfer_matrix, fr, recovery_idx, G,...
...
transfer_matrix = transfer_matrix + eye(num_spatial_modes); % now it goes (num_spatial_modes, num_spatial_modes)
...
% Gain term
gain_rhs = (G.*ifft(A_t)).'; % (num_modes,N) matrix, each column is a single w value
if ~sim.scalar
    gain_rhs = cat(3,gain_rhs(1:2:num_modes-1,:),gain_rhs(2:2:num_modes,:));
end
if sim.gpu_yes
    gain_term = permute(pagefun(@mldivide, transfer_matrix,gain_rhs),[2 1 3]);
else
    if sim.single_yes
        gain_term = complex(zeros(N, num_spatial_modes, polar, 'single'));
    else
        gain_term = complex(zeros(N, num_spatial_modes, polar));
    end
    for polar_idx = 1:polar
        gain_term(:, :, polar_idx) = (transfer_matrix(:, :, polar_idx)\gain_rhs(:,:,polar_idx)).';
    end
end
...
dAdz = fft(prefactor.*ifft(nonlinear) + gain_term);
