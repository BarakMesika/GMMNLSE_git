function calc_SRSK_tensors_fun(data)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%% Set parameters
modes_used = 1:data.num_modes;                      % In general the modes do not have to be consecutive
num_modes = length(modes_used);         % number of modes for which the tensors should be calculated
Nx = data.Nx;                              % number of spatial grid points for each mode
linear_yes = data.tenson_calc.linear_yes;                         % 1 = linear polarization, 0 = circular polarization
gpu_yes = data.tenson_calc.gpu_yes;                            % 1 = run on GPU, 0 = run on CPU
single_yes = data.tenson_calc.single_yes;                         % 1 = single precision, 0 = double precision
dir_prefix = data.folder_name; % folder containing the calculated modes

% File name parameters, as strings:
lambda0 = num2str(data.lambda0 * 1e9); % center wavelength in nm
radius = num2str(data.radius);
boundary = '0000';
...
for ii = 1:num_modes
   name = [dir_prefix sep_char 'radius', strrep(num2str(radius), '.', '_'), 'boundary' boundary 'fieldscalarmode',num2str(modes_used(ii),'%03.f'),'wavelength', lambda0, '.mat'];
   load(name, 'phi');
   fields(:, :, ii) = phi;
   norms(ii) = sqrt(sum(sum(abs(phi).^2)));
   disp(['Loaded mode ', int2str(modes_used(ii))])
end
...
thresholdzero=SR(1)*1e-4; % This is fairly arbitrary
