...
lrange = 0e-9; % wavelength range, in m. If 0 only the center wavelength will be used
num_modes = 100; % number of modes to compute
radius = 50; % outer radius of fiber, in um
folder_name = 'XLMA_YTF_100_400_480_100modes_at_1030nm'; % folder where the output will be stored

Nx = 1200; % number of spatial grid points
spatial_window = 200; % full spatial window size, in um
profile_function = @build_step; % function that builds the fiber
extra_params.NA = 0.11; % fiber NA
...
for kk = 1:length(l)
    ...
    for ii=1:num_modes
        ...
        fname=[folder_name sep_char 'radius'  num2str(radius) 'boundary' boundary 'field' field  'mode' num2str(ii, '%.3d') 'wavelength' num2str(round(lambda*1000))];
        print(gg,fname,'-dpng')
