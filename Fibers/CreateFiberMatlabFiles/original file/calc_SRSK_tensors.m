...
%% Set parameters
modes_used = 1:28;                      % In general the modes do not have to be consecutive
num_modes = length(modes_used);         % number of modes for which the tensors should be calculated
Nx = 2200;                              % number of spatial grid points for each mode
linear_yes = 1;                         % 1 = linear polarization, 0 = circular polarization
gpu_yes = 1;                            % 1 = run on GPU, 0 = run on CPU
single_yes = 0;                         % 1 = single precision, 0 = double precision
dir_prefix = 'GIF625_28modes_CW1030nm'; % folder containing the calculated modes
...
for ii = 1:num_modes
   name = [dir_prefix sep_char 'radius', radius, 'boundary' boundary 'fieldscalarmode',num2str(modes_used(ii),'%03.f'),'wavelength', lambda0, '.mat'];
%    name = [dir_prefix sep_char 'radius', radius, 'boundary' boundary 'fieldscalarmode',num2str(modes_used(ii)),'wavelength', lambda0, '.mat'];
   load(name, 'phi');
   fields(:, :, ii) = phi;
