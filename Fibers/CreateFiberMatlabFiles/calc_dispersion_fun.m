function calc_dispersion_fun(data)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
Nf = data.Nf;                                     % how many frequency points
lambda0_build = data.lambda0;                     % [m] the center wavelength used to solve for the modes
lambda0_disp = data.lambda0;                      % [m] the center wavelength at which the dispersion parameters will be evaluated
lrange = data.lrange;                             % [m] wavelength range. Should be the same as the range used to calculate the modes
modes_list=1:data.num_modes;                             % usually just 1:num_modes, but it can be different
                
polynomial_fit_order = data.dispersion.polynomial_fit_order;                    % polyfit order
num_disp_orders = data.dispersion.num_disp_orders;                         % i.e. if this is 3, 4 coefficients will be calculated, including the 0th order
                
radius = data.radius;                                 % Only needed for the file name
folder_name = data.folder_name;     % fiber folder name
...
for kk = 1:Nf
    lambda=l(kk);
    for ii=1:num_modes
        % fname=[folder_name sep_char 'radius' num2str(round(radius)) 'boundary' boundary 'field' field  'mode' num2str(modes_list(ii),'%03.f') 'wavelength' num2str(round(lambda*1000),'%04.f')];
        
        % added by BarakM 11.5.24
        fname=[folder_name sep_char 'radius' strrep(num2str(radius), '.', '_') 'boundary' boundary 'field' field  'mode' num2str(modes_list(ii),'%03.f') 'wavelength' strrep(num2str(lambda*1000), '.', '_')];
