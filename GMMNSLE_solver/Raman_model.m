function [fiber,haw,hbw] = Raman_model( fiber,sim,N,dt)
%RAMAN_MODEL It calculates the Raman response of several materials.
%   Input:
%       fiber.fiber_type: a string; the type of the fiber;
...
%           sim.single_yes
%           sim.gpu_yes
%       N: the number of time points in the simulation
%       dt: the time interval (N*dt=time window)
%
%   Output:
%       fiber: Besides "fiber_type", "fiber.fr" is included based on the material
...
hb = [];
switch fiber.fiber_type
    case 'silica'
        switch sim.Raman_model
            case 0 % ignore Raman effect
                fiber.fr = 0;
                ha = 0;
            case 1 % isotropic Raman
    ...
    case 'H2'
        switch sim.Raman_model
            ...
            case 1 % isotropic Raman
                fiber.fr = 1e-5;
                t1 = 8e-3/2/pi; % Raman parameter t1 [ps]
                t2 = 1e3; % Raman parameter t2 [ps]

                ha = ((t1^2+t2^2)/(t1*t2^2)).*exp(-t_shifted/t2).*sin(t_shifted/t1);
                peak_HR = max(imag(ifft(ha))*N*dt);
                ha = ha/fiber.fr*6.6e-11/2/peak_HR*fiber.H2_gR*1e18;
        end
        
end

% isotropic Raman
haw = ifft(ha)*N; % The factor of N is needed and used later in the convolution theorem because of how ifft is defined.
...
% anisotropic Raman
if ~isempty(hb)
    hbw = ifft(hb)*N; % The factor of N is needed and used later in the convolution theorem because of how ifft is defined.
                      % The convolution theorem is PQ=F[(p*q)]/N, where (p*q) is the discrete-form convolution, for discrete Fourier transforms.
                      % Normal (integral-form) convolution is p*q=(p*q)*dt.
    if sim.single_yes
        hbw = single(hbw);
