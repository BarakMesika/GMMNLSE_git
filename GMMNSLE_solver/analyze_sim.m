...
%   fig = func.analyze_fields_within_cavity(t,f,fields,saved_z,splice_z,midx);
%   fig = func.analyze_ellipticity_within_cavity(fields,saved_z,ellipticity);
%   fig = analyze_gain_within_cavity(saved_z,splice_z,pump,N2);
%   fig = analyze_ASE_within_cavity(f,ASE,saved_z,splice_z,midx);
%   fig = func.animation_within_cavity(t,f,fields,saved_z,varargin);

func.extract_saved_field               = @extract_saved_fields;
func.analyze_fields_within_cavity      = @analyze_fields_within_cavity;
func.analyze_ellipticity_within_cavity = @analyze_ellipticity_within_cavity;
func.analyze_gain_within_cavity        = @analyze_gain_within_cavity;
func.analyze_ASE_within_cavity         = @analyze_ASE_within_cavity;
func.animation_within_cavity           = @animation_within_cavity;
...
%%
function fig = analyze_fields_within_cavity(t,f,fields,saved_z,splice_z,midx)
...
% "real" is just to suppress the warning if calc_RMS gives a warning about
% its imaginary output.
% If the input is really noisy, the calculated RMS can be imaginary.
duration = real(permute(calc_RMS(t,intensity),[3 2 1])); % ps
bandwidth = real(permute(calc_RMS(wavelength(positive_wavelength),spectrum(positive_wavelength,:,:)),[3 2 1])); % nm
...
%%
function fig = analyze_ellipticity_within_cavity(fields,saved_z,ellipticity)
...
[ellipticity_phi_within_cavity,ellipticity_theta_within_cavity] = calc_ellipticity( fields,ellipticity );
[~,peak_idx] = max(abs(fields(:,1,:)).^2);
peak_idx = peak_idx + permute(0:N:(N*length(peak_idx)-1),[1 3 2]);
peak_phi = squeeze(ellipticity_phi_within_cavity(peak_idx));
peak_theta = squeeze(ellipticity_theta_within_cavity(peak_idx));
...
%%
function fig = analyze_gain_within_cavity(saved_z,splice_z,pump,N2)
...
h = plot(saved_z,permute(pump,[3 1 2]));
...
hold on;
for i = 1:length(splice_z)
    h = plot(splice_z(i)*[1 1],[min(pump) max(pump)]);
    set(h,'linewidth',2,'linestyle','--','color','black','marker','none');
end
hold off;
...
%%
function fig = analyze_ASE_within_cavity(f,ASE,saved_z,splice_z,midx)
...
if length(midx) == 1
    ...
    set(h,'linewidth',2); set(h2,'linewidth',2); set(gca,'fontsize',14);
    for i = 1:length(splice_z)
...
%%
function fig = animation_within_cavity(t,f,fields,saved_z,varargin)
...
%       tlim - (1,2) matrix; the range of the time to plot (default: [])
%       wavelengthlim - (1,2) matrix; the range of the wavelength to plot (default: [])
...
for i = 1:save_point
    ...
    window = 2^nextpow2(N/30); % the window for each short-time-Fourier-Transform
                               % I choose to have about 30 windows here.
    noverlap = floor(window*0.9); % the overlap of the window
                                  % I choose to have 90% overlap.
    nfft = 2^nextpow2(window); % the number of points for fft
    Fs = 1/dt; % the sampling rate
    [~,f_tmp,t_tmp,psd] = spectrogram(conj(fields(:, midx, i)),window,noverlap,nfft,Fs,'yaxis');
    psd = fftshift(psd,1);
    ...
    t_spectrogram = interp1(1:N,t,linspace(1,N,length(t_tmp)))';
    f_spectrogram = interp1(1:N,f,linspace(1,N,length(f_tmp)))';
