function [converged_yes,force_stop_yes,fig] = check_convergence( energy,field,freq,t,varargin)
...
converged_yes = false;
force_stop_yes = struct('convergence',false,'fluctuation',false);
...
plot_yes = true;
force_stop = struct('tol_convergence',1e-4,...
                    'tol_fluctuation',5e-4,...
                    'min_rt',30);
reset_idx = 1;
optargs = {tol_convergence,plot_yes,force_stop,reset_idx};
...
[tol_convergence,plot_yes,force_stop,reset_idx] = optargs{:};
...
if rt_num > least_rt
    ...
    if ~any(abs(considered_difference) > tol_convergence)
        ...
        converged_yes = true;
    elseif rt_num >= force_stop.min_rt
        tol_close_to_pulse = 0.01;
        
        idx_close_to_pulse = find(difference < tol_close_to_pulse,1)+1;
        if isempty(idx_close_to_pulse) || idx_close_to_pulse > rt_num-4 % it fluctuates too much to even close to a steady-state pulse
            force_stop_yes.fluctuation = true;
        else
            average_energy = cumsum(energy(idx_close_to_pulse:end))./(1:length(energy(idx_close_to_pulse:end)))';
            diff_average = abs(relative_diff(average_energy));
            if ~any(diff_average(end-2:end) > force_stop.tol_convergence) % fluctuation can be tolerated
                force_stop_yes.convergence = true;
            elseif any(diff_average(end) > force_stop.tol_fluctuation) % fluctuation can't be tolerated
                force_stop_yes.fluctuation = true;
            end
        end
    end
