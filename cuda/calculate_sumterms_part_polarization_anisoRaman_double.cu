__global__ void calculate_sumterm_part(double2 * Kerr, double2 * Ra, double2 * Rb, double2 * transfer_matrix, const double2 * A_t, const double2 * Bmn, const double* SRa, const unsigned char* SRa_nonzero_midx1234s, const double* SRb, const unsigned char* SRb_nonzero_midx1234s, const double* SK, const unsigned char* SK_nonzero_midx1234s, const unsigned int N, const unsigned int M, const unsigned int NUM_SRa_NONZERO, const unsigned int NUM_SK_NONZERO, const unsigned int NUM_MODES) {
    ...
    for (int i = 0; i < NUM_SK_NONZERO; i++) {
        ...
        double SK_a = A_t[Nidx+Midx*N+SK_midx2*NM].x;
        double SK_b = A_t[Nidx+Midx*N+SK_midx2*NM].y;
        double SK_c = A_t[Nidx+Midx*N+SK_midx3*NM].x;
        double SK_d = A_t[Nidx+Midx*N+SK_midx3*NM].y;
        double SK_e = A_t[Nidx+Midx*N+SK_midx4*NM].x;
        double SK_f = A_t[Nidx+Midx*N+SK_midx4*NM].y;
        ...
        double SRb_c = A_t[Nidx+Midx*N+SRb_midx3*NM].x;
        double SRb_d = A_t[Nidx+Midx*N+SRb_midx3*NM].y;
        double SRb_e = A_t[Nidx+Midx*N+SRb_midx4*NM].x;
        double SRb_f = A_t[Nidx+Midx*N+SRb_midx4*NM].y;
        ...
        if (i < NUM_SRa_NONZERO) {
            ...
            double SRa_c = A_t[Nidx+Midx*N+SRa_midx3*NM].x;
            double SRa_d = A_t[Nidx+Midx*N+SRa_midx3*NM].y;
            double SRa_e = A_t[Nidx+Midx*N+SRa_midx4*NM].x;
            double SRa_f = A_t[Nidx+Midx*N+SRa_midx4*NM].y;
