__global__ void calculate_sumterm_part(double2 * Kerr, double2 * transfer_matrix, const double2 * A_t, const double2 * Bmn, const double* SRa, const unsigned char* SRa_nonzero_midx1234s, const double* SK, const unsigned char* SK_nonzero_midx1234s, const unsigned int N, const unsigned int M, const unsigned int NUM_SRa_NONZERO, const unsigned int NUM_SK_NONZERO, const unsigned int NUM_MODES) {
    ...
    for (int i = 0; i < NUM_SK_NONZERO; i++) {
        ...
        // Because NUM_SK_NONZERO >= NUM_SRa_NONZERO, I calculate SR tensors inside SK for-loop.
